...
COMOBJECTS="jaricom jcomapi jutils jerror jmemmgr jmemnobs"

CLIBOBJECTS="jcapimin jcapistd jcarith jctrans jcparam \
        jdatadst jcinit jcmaster jcmarker jcmainct jcprepct \
        jccoefct jccolor jcsample jchuff jcdctmgr jfdctfst \
        jfdctflt jfdctint"

DLIBOBJECTS="jdapimin jdapistd jdarith jdtrans jdatasrc \
        jdmaster jdinput jdmarker jdhuff jdmainct \
        jdcoefct jdpostct jddctmgr jidctfst jidctflt \
        jidctint jdsample jdcolor jquant1 jquant2 jdmerge"
...
function build_jpegtran() {
  do_compile . "${COMOBJECTS} ${CLIBOBJECTS} ${DLIBOBJECTS} jpegtran transupp cdjpeg rdswitch" ""
  if [ $? != 0 ]; then
    return $?
  fi
  cd _build
